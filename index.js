...
async function refresh() {
  try {
    ...
    let hourly = ''
    for (let i = 0; i < 7; i++){
      weekly += `${unixToDateTime(data.daily.data[i].time)} ${Math.round(data.daily.data[i].temperatureHigh)}/${Math.round(data.daily.data[i].temperatureLow)} ${data.daily.data[i].precipProbability*100}% ${data.daily.data[i].precipType}\n`
    }
    for (let i = 0; i < 24; i++){
      hourly += `${unixToHours(data.hourly.data[i].time)} ${Math.round(data.hourly.data[i].temperature)}F ${data.hourly.data[i].precipProbability*100}% ${data.hourly.data[i].precipType}\n`
    }

    const text = `${unixToDateTime(time)} - ${summary}
